...
public class SmaliCodeModel implements CodeModel {
	...
	public String[] getDefaultFilePatterns() {
		return new String[]{"*.smali", "*.json", "*.cmake", "*CMakeLists.txt", "*.properties", "*.yaml", "*.toml"};
	}
	...
	public void fillSyntaxTree(FileEntry fileEntry, Reader reader, Map<Language, SyntaxTreeStyles> map) {
		...
		this.zeroAicyHighlighter.highlight(fileEntry, reader, map.get(language));
	...
	public List<Language> getLanguages() {
		List<Language> list = new ArrayList<>(1);
		list.add(language);
		return list;
	}
}
